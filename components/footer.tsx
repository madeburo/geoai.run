...
export function Footer() {
  ...
  return (
    <footer className="border-t border-border/30 bg-background px-4 sm:px-6 dark:border-white/4">
      <div className="mx-auto flex max-w-6xl flex-col items-center gap-8 py-12 text-center sm:py-16 md:flex-row md:justify-between md:text-left">
        <div className="flex flex-col items-center md:items-start gap-2">
          ...
          <p className="text-xs text-muted-foreground/60 sm:text-sm">{tFooter("tagline")}</p>
        </div>
          ...
          {NAV_LINKS_CONFIG.map((link) =>
            link.external ? (
              <a
                ...
                className="text-xs text-muted-foreground/60 transition-colors hover:text-foreground sm:text-sm"
              >
        ...
        <p className="text-xs text-muted-foreground/40 sm:text-sm">{tFooter("copyright")}</p>
      </div>
    </footer>
  );
}